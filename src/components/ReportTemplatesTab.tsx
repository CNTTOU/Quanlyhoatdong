...
const emptyConfig: ReportTemplateConfig = {
  reportTypes: [],
  units: [],
  templates: [],
  dataFields: [],
...
export function ReportTemplatesTab() {
  const [config, setConfig] = useState<ReportTemplateConfig>(emptyConfig);
  const [selectedTemplate, setSelectedTemplate] = useState<number | null>(null);
  const [searchQuery, setSearchQuery] = useState('');

  useEffect(() => {
    getInterfaceDocument<ReportTemplateConfig>('cau_hinh_mau_bao_cao', emptyConfig).then((data) => {
      setConfig(data);
  ...
  const selected = config.templates.find(t => t.id === selectedTemplate);
              ...
              <div className="space-y-4">
                ...
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <div className="space-y-2">
                    {config.templateStructure.map((section, index) => (
                ...
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {config.dataFields.slice(0, selected.fields).map((field, index) => (
                      <span
                        key={index}
