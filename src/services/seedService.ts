...
export async function seedPermissions() {
  await Promise.all(
    PERMISSIONS.map(([ma_quyen, ten_quyen, nhom_quyen, la_quyen_nguy_hiem]) =>
      setDoc(
...
export async function seedRoles() {
  await Promise.all(
    ROLES.map(([ma_vai_tro, ten_vai_tro, mo_ta, cap_do]) =>
      setDoc(
  ...
  await Promise.all(
    DEFAULT_REPORT_TEMPLATES.map((template) =>
      setDoc(
        ...
        {
          ...template,
          don_vi_ap_dung: ['doan_khoa', 'lien_chi_hoi', 'chi_doan', 'chi_hoi', 'cau_lac_bo', 'doi_nhom'],
          ho_tro_word: true,
          ho_tro_pdf: true,
...
export async function seedDefaultNotification() {
  await setDoc(
    doc(db, 'thong_bao', 'mau_thong_bao_he_thong'),
    {
      ...
      noi_dung: 'Đây là mẫu thông báo hệ thống dùng để kiểm tra collection thong_bao.',
...
export async function seedInterfaceDisplayData() {
  await setDoc(
    doc(db, 'du_lieu_cong_khai', 'hoat_dong_noi_bat'),
    {
      ...PUBLIC_FEATURED_DATA,
      ngay_cap_nhat: serverTimestamp(),
    },
    { merge: true },
  );
  ...
  await Promise.all(
    INTERFACE_DISPLAY_SEED_DOCS.map((item) =>
      setDoc(
        doc(db, 'du_lieu_hien_thi', item.id),
        {
          ...item.data,
          ngay_cap_nhat: serverTimestamp(),
        },
...
export async function seedAll() {
  ...
  await seedInterfaceDisplayData();
